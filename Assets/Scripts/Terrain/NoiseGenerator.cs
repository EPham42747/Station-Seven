...
public static class NoiseGenerator {
    ...
    /// <param name="amplitude">The positive number by which the noise is scaled. Should be positive.</param>
    /// <param name="minimum">A minimum for the floats. All values below this number are set to this value. Should be positive.</param>
    ...
    public static float[,] GenerateNoise(int width, int height, float frequency = 1f, float amplitude = 1f, float minimum = 0f, float sharpness = 1f) {
        if (frequency <= 0f) frequency = 0.001f;
        if (amplitude < 0f) amplitude = 0f;
        if (minimum < 0f) minimum = 0f;
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ret[i, j] = Mathf.Pow(Mathf.PerlinNoise(i * frequency + offset, j * frequency + offset), sharpness) * amplitude;
                ret[i, j] = Mathf.Max(ret[i, j], minimum);
            }
        }
